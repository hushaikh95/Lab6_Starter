...
`;
	...
	set data(data) {
		...
		const r = Math.min(5, Math.max(0, Math.round(Number(data.rating)) || 0));

		article.innerHTML = `
  <img src="${escapeHtml(data.imgSrc)}"
    alt="${escapeHtml(data.imgAlt)}">
  <p class="title">
    <a href="${escapeHtml(data.titleLnk)}">${escapeHtml(data.titleTxt)}</a>
  </p>
  <p class="organization">${escapeHtml(data.organization)}</p>
  <div class="rating">
    <span>${r}</span>
    <img src="./assets/images/icons/${r}-star.svg" alt="${r} stars">
    <span>(${data.numRatings})</span>
  </div>
  <time>${escapeHtml(data.lengthTime)}</time>
  <p class="ingredients">
